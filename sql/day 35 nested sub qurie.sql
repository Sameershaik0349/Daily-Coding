...
-- fro specifc salaary
select salary from emp order by salary desc limit 19,1;



select * from emp where department =
(SELECT department from emp where salary =
(select max(salary) from emp where salary >
(select avg(salary) from emp)));

-- "corelated quriesssss" ------- will take help of outer qureiy to form condition

-- when groupin of values given for less than or greater quriey sure use correlated qurie
